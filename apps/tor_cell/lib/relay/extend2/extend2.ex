defmodule TorCell.Relay.Extend2 do
  defstruct specs: nil,
            type: nil,
            data: nil

  defp fetch_specs(specs, n, payload) when n > 0 do
    {spec, payload} = TorCell.Relay.Extend2.Spec.fetch(payload)
  ...
  def decode(payload) do
    ...
    %TorCell.Relay.Extend2{specs: specs, type: :ntor, data: data}
  end

  # TODO: Encode this
  def encode(cell) do
    <<length(cell.specs)>> <>
      Enum.join(Enum.map(cell.specs, fn x -> TorCell.Relay.Extend2.Spec.encode(x) end)) <>
      <<0x02::16>> <>
